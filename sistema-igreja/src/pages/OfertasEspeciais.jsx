...
import { getOfertasEspeciais, createOfertaEspecial, deleteOfertaEspecial } from '../services/ofertasEspeciaisService'
...
function OfertasEspeciais() {
  ...
  async function carregarTudo() {
    setCarregando(true)
    try {
      const [ofertasRes, membrosRes] = await Promise.all([getOfertasEspeciais(), getMembros()])
      if (!ofertasRes.error) setOfertas(ofertasRes.data || [])
      else showToast('Erro ao carregar: ' + ofertasRes.error.message, 'error')
  ...
  async function salvar() {
    const novosErros = {}
    if (!form.motivo.trim()) novosErros.motivo = 'Informe o motivo/evento'
    ...
    try {
      ...
      const { error } = await createOfertaEspecial(dados)
      if (error) { showToast('Erro ao registrar: ' + error.message, 'error'); return }

      showToast('Oferta especial registrada com sucesso', 'success')
      await carregarTudo()
      setForm(initialForm)
      setErros({})
      setModalAberto(false)
    } catch {
      showToast('Erro ao registrar oferta especial', 'error')
    } finally {
      setSalvando(false)
    }
  }
  ...
  async function excluir(id) {
    if (!window.confirm('Excluir esta oferta especial?')) return
    try {
      const { error } = await deleteOfertaEspecial(id)
      if (error) { showToast('Erro ao excluir: ' + error.message, 'error'); return }
      showToast('Oferta especial excluída com sucesso', 'success')
  ...
  return (
    <div className="page-entradas">
      ...
      <div className="entradas-header">
        <div>
          <h2>Registro de Ofertas Especiais</h2>
        ...
        <button onClick={abrirModal} className="btn-cadastrar" disabled={carregando}>
          + Registrar oferta especial
        </button>
      </div>

      <div className="totais-row">
        <div className="total-card total-mes">
          <span className="total-label">Ofertas especiais este mês</span>
      ...
      {ofertas.length === 0 ? (
        <div className="sem-resultados"><p>Nenhuma oferta especial registrada</p></div>
      ) : (
            ...
            <tbody>
              {ofertas.map(o => (
                <tr key={o.id}>
                  ...
                  <td><span className="badge-motivo">{o.motivo}</span></td>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={ev => ev.stopPropagation()}>
            <h3>Registrar Oferta Especial</h3>

            <div className="form-grid">
              <div className="form-group full">
                <label>Motivo / Evento <span className="required-star">*</span></label>
                <input
                  name="motivo"
                  value={form.motivo}
                  onChange={handleChange}
                  placeholder="Ex: Natal, Páscoa, Construção..."
              ...
              <div className="form-group full">
                ...
                <input
                  ...
                  onChange={handleChange}
                  list="lista-membros-esp"
                  placeholder="Digite o nome..."
                  autoComplete="off"
                />
                <datalist id="lista-membros-esp">
